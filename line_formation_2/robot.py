...
class Robot:
    ...
    def update_state(self, all_robots):
        ...
        # last_goal = len(self.sim_params["GOALS_POS"]) - 1 #NOVO         #APAGAR
        is_last_goal = self.target_index == len(self.sim_params["GOALS_POS"]) - 1     #NOVO2
        if not is_last_goal:                #NOVO2
            if any(
                dist_to_goal < radius
                ...
                for r in robots_w_same_goal:
                    ...
                    r.inline_following_robot = None     #self ou .r CORRIGIDO
        ...
        #         reach = self.sim_params["REACH_GOAL_RADIUS"]
        # if self.target_index == last_goal:
        #     reach = max(reach, self.separation_radius)  # ex: 10 em vez de 5

        # if dist_to_goal <= reach:
        #     if self.target_index < last_goal:
        #         self.target_index += 1
        #         self.state = RobotStatus.GROUP
        #         self.inline_following_robot = None
        ...
        #         # "snap" pro ponto do goal (fica preto em cima do X roxo)
        #         self.position = curr_goal_pos.copy()
        #         self.front_position = self.position.copy()

        #     return



        R0 = self.rearranging_regions_radii[0]
        if dist_to_goal > R0:
            # saiu da zona de congestionamento -> volta pro group
            self.state = RobotStatus.GROUP
            self.inline_following_robot = None
        ...
        # if self.target_index == last_goal:
        #     # No último goal: não formar fila (evita leader infinito)              # NOVO      #APAGAR
        #     self.state = RobotStatus.GROUP                                                          #NOVO
        #     self.inline_following_robot = None
        #     return
        if not is_last_goal: #NOVO2
            robots_w_same_goal = [
    ...
    def calculate_resultant_velocity(self, all_robots, obstacles):
        ...
        alpha = 1  # Default for 'group' state (combines PF and Flocking) 
        target_for_pf = curr_goal_pos  # Default attractive target for PF

        if self.state == RobotStatus.LEADER:
            alpha = 0  # No flocking for leader 
            target_for_pf = curr_goal_pos  # Leader moves directly to the target 
        elif self.state == RobotStatus.IN_LINE:
            alpha = 0  # No flocking for inLine robots 
